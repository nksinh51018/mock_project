...
const reducer = (state = initialState, action) => {
  switch (action.type) {
      ...
      if (indexProduct2 === -1) {
        ...
        if (state.products[indexProduct2].type === 1) {
          if (
            ...
            state.products[indexProduct2].quantity
          ) {
            state.products[indexProduct2].amount++;
          } else {
            message.error(
              `${state.products[indexProduct2].name} đã tối đa số lượng`
            );
          }
        } else {
          message.warning(
            `Dịch vụ ${state.products[indexProduct2].name} đã tồn tại`
          );
        }
      }
      return { ...state };
    case CreateMaintenanceCardType.REMOVE_PRODUCT:
      ...
      let indexProductKey = findItem(
        idProductRemove.toString(),
        state.products
      );
    ...
    case CreateMaintenanceCardType.CHANGE_AMOUNT:
      ...
      if (state.products[indexProduct1].quantity >= action.payload.value) {
        ...
        state.products[indexProduct1].amount =
          state.products[indexProduct1].quantity;
        message.error(
          `${state.products[indexProduct1].name} còn lại ${state.products[indexProduct1].quantity} sản phẩm`
        );
      }
    ...
    case CreateMaintenanceCardType.WARRANTY_PRODUCT:
      ...
      const indexProductWarranty = findItem(
        idProductWarranty.toString(),
        state.products
      );
    ...
    case CreateMaintenanceCardType.GET_MAINTENANCE_CARD_BY_HMAC_SUCCESS:
      state.check = true;
      state.customerItem = action.payload.data.customer;
      state.products = [];
      for (
        let i = 0;
        i < action.payload.data.maintenanceCardDetails.length;
        i++
      ) {
        let maintenanceCardDetail =
          action.payload.data.maintenanceCardDetails[i];
      ...
      state.maintenanceCardDetailStatusHistories =
        action.payload.data.maintenanceCardDetailStatusHistories;
      ...
      if (action.payload.data.repairman === null) {
        ...
        state.repairman = action.payload.data.repairman;
      }
      if (action.payload.data.repairman !== null) {
      ...
      state.info.expectedReturnDate = action.payload.data.expectedReturnDate;
      state.id = action.payload.data.id;
    ...
    case CreateMaintenanceCardType.UPDATE_MAINTENANCE_CARD_SUCCESS:
      ...
      state.info = {
        ...
        expectedReturnDate: action.payload.data.expectedReturnDate,
      };
      if (action.payload.data.repairman === null) {
        state.repairman = {};
      } else {
    ...
    case CreateMaintenanceCardType.GET_PLATE_NUMBER_BY_CUSTOMER_SUCCESS:
      ...
      return { ...state };
    case CreateMaintenanceCardType.GET_HMAC_SUCCESS:
      state.isOpenQrCode = true;
      state.hmac = action.payload.data;
      return { ...state };
    case CreateMaintenanceCardType.SET_RETURN_DATE:
      state.info.returnDate = new Date();
      return { ...state };
    default:
      return { ...state };
  }
